...
-- JOIN STMNT(S)
SELECT m.name, c.committee_name AS 'Committeee Assignment'
FROM members m JOIN committees c ON m.committee_id = c.committee_id;

SELECT '2' AS 'INNER JOIN STATEMENTS' \G

-- INNER JOIN STMNT(S)
SELECT m.name, c.committee_name AS 'Committee Assignment'
FROM members m INNER JOIN committees c ON m.committee_id = c.committee_id;

SELECT '3' AS 'LEFT JOIN STATEMENTS' \G

-- LEFT JOIN STMNT(S)
SELECT m.name, c.committee_name AS 'Committee Assignment'
FROM members m LEFT JOIN committees c ON m.committee_id = c.committee_id;

SELECT '4' AS 'ANTI LEFT JOIN STATEMENTS' \G

-- ANTI LEFT JOIN STMNT(S)
SELECT m.name, c.committee_name AS 'Committee Assignment'
FROM members m LEFT JOIN committees c ON m.committee_id = c.committee_id WHERE c.committee_id IS NULL;
...
-- RIGHT JOIN STMNT(S)
SELECT m.name, c.committee_name AS 'Committee Assignment'
FROM members m RIGHT JOIN committees c ON m.committee_id = c.committee_id;

SELECT '6' AS 'ANTI RIGHT JOIN STATEMENTS' \G

-- ANTI RIGHT JOIN STMNT(S)
SELECT m.name, c.committee_name AS 'Committee Assignment'
FROM members m RIGHT JOIN committees c ON m.committee_id = c.committee_id WHERE m.committee_id IS NULL;
